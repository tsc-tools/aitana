...
def test_gas():
    ...
    assert (df1.obs.min() / 86.4 - 1.192) < 0.001
...
def test_seismicity():
    ...
    assert dfq.magnitude.max() > 4.0
    assert dfq.magnitude.min() < 2.0
...
def test_load_all():
    end_date = "2025-11-26"
    df = load_all(end_date=end_date, fill_method=None)
    assert df.SO2[~pd.isnull(df.SO2)].index[-1] == pd.Timestamp(
        "2025-11-04 00:00:00+0000", tz="UTC"
    )
    assert df.SO2[~pd.isnull(df.SO2)][-1] - 839.81 < 0.01
    # Check that caching works with function default arguments
    df = load_all()
    assert len(df) > 0
